...
{
	CSC2110::ListArrayIterator<Player>* player_iter = players->iterator();
...
void Players::occupyCity(uint8 player_id, CSC2110::ListArray<CSC2110::Integer>* token_types, uint8 worker_cost)
{
	Player* p = players->get(player_id);
	CSC2110::ListArrayIterator<CSC2110::Integer>* token_types_iter = token_types->iterator();
	while(token_types_iter->hasNext())
	{
		CSC2110::Integer* token_type = token_types_iter->next();
...
EndeavorBuildingType Players::findSelectedActionToken(uint8 player_id, uint32 x, uint32 y)
{
   ...
   CSC2110::ListArrayIterator<HotSpot>* action_token_iter = action_token_hot_spots->iterator();
...
uint8 Players::findSelectedPlayer(uint32 x, uint32 y)
{
   ...
   CSC2110::ListArrayIterator<HotSpot>* player_iter = player_view_hot_spots->iterator();
   while(player_iter->hasNext())
   {
...
void Players::getActivatedBuildings(CSC2110::ListArray<Building>* activated_buildings, uint8 player_id)
{
   Player* p = players->get(player_id);
...
Players::Players(PixelXML& pixel_xml, ImageLoader* image_loader, Buildings* buildings)
{
	...
	players = new CSC2110::ListArray<Player>();
   ...
   PixelPoint* pixel_point_colonial_house = player_building_locations->get(1);
	...
	CSC2110::ListArray<PixelPoint>* player_view_locations = pixel_xml.getPixelCentersList("player_view_locations");
	PixelDimension pixel_view_dim = pixel_xml.getPixelDimension("player_view_locations");
	player_view_hot_spots = new CSC2110::ListArray<HotSpot>();
	for (int i = 1; i <= 5; i++)
	{
	...
	CSC2110::ListArrayIterator<PixelPoint>* point_view_iter = player_view_locations->iterator();
	...
	CSC2110::ListArray<PixelPoint>* action_token_locations = pixel_xml.getPixelCentersList("action_token_locations");
	PixelDimension action_token_dim = pixel_xml.getPixelDimension("action_token_locations");
	action_token_hot_spots = new CSC2110::ListArray<HotSpot>();
	for (int i = 1; i <= 4; i++)
	{
	...
	CSC2110::ListArrayIterator<PixelPoint>* action_token_iter = action_token_locations->iterator();
	while(action_token_iter->hasNext())
	{
   ...
   CSC2110::Random* rand = CSC2110::Random::getRandom();
...
Players::~Players()
{
	...
	CSC2110::ListArrayIterator<HotSpot>* action_token_hot_iter = action_token_hot_spots->iterator();
	...
	CSC2110::ListArrayIterator<HotSpot>* player_view_iter = player_view_hot_spots->iterator();
	...
	CSC2110::ListArrayIterator<PixelPoint>* player_card_iter = player_card_locations->iterator();
	...
	CSC2110::ListArrayIterator<PixelPoint>* player_info_iter = player_info_locations->iterator();
	...
	CSC2110::ListArrayIterator<PixelPoint>* track_locs_iter = player_track_locations->iterator();
   ...
   CSC2110::ListArrayIterator<PixelPoint>* iter_building = player_building_locations->iterator();
   ...
   CSC2110::ListArrayIterator<Player>* iter_player = players->iterator();
