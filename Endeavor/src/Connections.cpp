...
uint8 Connections::computeConnectionsGlory(uint8 player_id)
{
	...
	CSC2110::BinaryTreeIterator<Connection>* conn_iter = connections_by_first_city->iterator();
...
Connections::Connections(PixelXML& pixel_xml, CSC2110::Permutation& permute, CSC2110::ListArray<Token>* tokens)
{
	//connections = new CSC2110::ListArray<Connection>();
	connections_by_first_city = new CSC2110::BinarySearchTree<Connection>(&Connection::compareConnectionsByFirstCity, &Connection::compareConnectionsByKeyAndFirstCity);
	connections_by_second_city = new CSC2110::BinarySearchTree<Connection>(&Connection::compareConnectionsBySecondCity, &Connection::compareConnectionsByKeyAndSecondCity);

	CSC2110::ListArray<PixelPoint>* connection_centers = pixel_xml.getPixelCentersList("connections");
	PixelDimension connections_dim = pixel_xml.getPixelDimension("connections");

	CSC2110::ListArrayIterator<PixelPoint>* connection_centers_iter = connection_centers->iterator();
...
CSC2110::ListArray<Connection>* Connections::getAllConnectionsContainingCityID(uint8 city_id)
{
	CSC2110::String sk = CSC2110::String::i_to_a(city_id);
	CSC2110::ListArray<Connection>* result_connections_1 = connections_by_first_city->retrieveDuplicates(&sk);
	CSC2110::ListArray<Connection>* result_connections_2 = connections_by_second_city->retrieveDuplicates(&sk);
	
	//merge the results together
	CSC2110::ListArrayIterator<Connection>* conn_iter = result_connections_2->iterator();
	while(conn_iter->hasNext())
	{
		Connection* conn = conn_iter->next();
...
//PRINT("Connections Destructor Called.")
	CSC2110::BinaryTreeIterator<Connection>* connections_iter = connections_by_first_city->iterator();
