...
class Player
{
   private:
      ...
      CSC2110::ListLinked<Building>* buildings;
   ...
   public:
	  ...
	  void abolishSlavery(CSC2110::ListArray<PixelPoint>* player_board_card_locs);
	  uint8 computePlayerBoardGlory();
	  void computeTotalGlory(uint8 board_and_connection_glory);
	  
	  Card* discard(uint8 card_id, CSC2110::ListArray<PixelPoint>* player_board_card_locs);
	  uint8 findSelectedCardToDiscard(uint32 x, uint32 y);
	  bool mustDiscard();
	  
	  void workerCost(uint8 cost);
	  void occupyCity(EndeavorTokenType token_type);
	  void takeSelectedCard(Card* card, CSC2110::ListArray<PixelPoint>* player_board_card_locs);
      ...
      void draw(wxDC&  dc, uint8 active_player, CSC2110::ListArray<PixelPoint>* track_locations, CSC2110::ListArray<PixelPoint>* info_locations);
      ...
      void getActivatedBuildings(CSC2110::ListArray<Building>* activated_buildings);
      uint8 getNumUnactivatedBuildings();
      uint8* getUnactivatedBuildings();
	  
	  TiXmlElement* writeXML();
      void readXML(TiXmlElement* player_element, EndeavorGame* game, CSC2110::ListArray<PixelPoint>* player_board_card_locs, CSC2110::ListArray<PixelPoint>* player_building_locations);
};
